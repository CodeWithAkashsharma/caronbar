...
import { MONTHLY_VEHICLE_TYPES } from '../data/vehicleTypes';
...
export const Home = () => {
  ...
  const is2Wheeler = activePackageType === 'monthly'
    ? monthlyVehicleId === '2wheeler'
    : (currentVehicle?.id === '2wheeler' || selectedVehicleId === '2wheeler');

  const packagesList = is2Wheeler
    ? BIKE_PACKAGES_DATA
    : (activePackageType === 'monthly' ? MONTHLY_PACKAGES_DATA : DAILY_PACKAGES_DATA);

  // Price resolver: monthly uses monthlyVehicleId, daily uses global selectedVehicleId
  const getPackagePrice = (pkg) => {
    if (!pkg.prices) {
      return activePackageType === 'monthly'
        ? Math.round(pkg.basePrice * (MONTHLY_VEHICLE_TYPES.find(v => v.id === monthlyVehicleId)?.multiplier || 1))
        : Math.round(pkg.basePrice * (currentVehicle?.multiplier || 1));
    }
    if (activePackageType === 'monthly') {
      return pkg.prices[monthlyVehicleId] ?? pkg.prices['hatchback-sedan'] ?? pkg.basePrice;
    }
    return pkg.prices[currentVehicle?.id] ?? pkg.prices['hatchback'] ?? pkg.basePrice;
  };

  const getPackageOriginalPrice = (pkg, currentPrice) => {
        ...
        <div className="max-w-7xl mx-auto space-y-7 sm:space-y-9 relative z-10">
          ...
          <div className="bg-white border border-slate-200 rounded-xl sm:rounded-2xl p-2 sm:p-6 max-w-4xl mx-auto shadow-xs sm:shadow-sm">
            {activePackageType === 'monthly' ? (
              // Monthly → controlled 3-tab merged selector
              <VehicleSelector
              ...
              // Daily → context 4-tab selector (Hatchback / Sedan / SUV / 2-Wheeler)
              <VehicleSelector />
            )}
          </div>
          ...
          {/* Packages Grid - Always Centered */}
          <div className="flex flex-wrap justify-center items-stretch gap-2.5 sm:gap-5 max-w-6xl mx-auto">
            {packagesList.map((pkg) => {
              const price = getPackagePrice(pkg);
              const originalPrice = getPackageOriginalPrice(pkg, price);
                ...
                <div
                  ...
                  className={`w-full ${
                    is2Wheeler
                      ...
                      : 'sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-0.9rem)] max-w-sm'
                  } group rounded-xl sm:rounded-2xl p-2.5 min-[360px]:p-3.5 sm:p-5 transition-all duration-300 ease-out flex flex-col justify-between relative hover:-translate-y-1.5 hover:shadow-xl cursor-pointer ${
                    pkg.popular || is2Wheeler
                      ? 'bg-white border-2 border-[#8B182B] shadow-lg hover:border-[#A61C33] hover:ring-2 hover:ring-[#8B182B]/20'
                      : 'bg-white border border-slate-200 hover:border-[#8B182B] shadow-xs'
                  }`}
                >
                  <div className="flex flex-col flex-1">
                    {(pkg.popular || is2Wheeler) && (
                      <span className="absolute -top-2.5 sm:-top-3.5 left-1/2 -translate-x-1/2 text-[7.5px] sm:text-[9.5px] font-black uppercase tracking-widest px-2.5 sm:px-3.5 py-0.5 rounded-full bg-[#8B182B] text-white shadow-md z-10 whitespace-nowrap group-hover:bg-[#A61C33] transition-colors">
                    ...
                    <div className="mb-1.5 sm:mb-2.5 pt-0.5">
                      <span className="text-[7.5px] sm:text-[9px] font-mono font-bold uppercase tracking-wider text-slate-400 block mb-0.5">
                        {activePackageType === 'monthly' ? 'SUBSCRIPTION PLAN' : 'ONE-TIME'}
                      </span>
                      ...
                      <p className="font-sans text-[8.5px] min-[360px]:text-[9.5px] sm:text-xs text-slate-500 leading-tight sm:leading-snug mt-0.5 line-clamp-2">
                        {pkg.tagline}
                      </p>
                    </div>
                    ...
                    <div className="p-1.5 sm:p-3 rounded-lg sm:rounded-xl bg-slate-50 border border-slate-200 mb-2 sm:mb-3.5 flex flex-col justify-between group-hover:bg-rose-50/30 transition-colors gap-1">
                      <div className="flex items-center justify-between gap-1">
                        ...
                        <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[6.5px] min-[360px]:text-[7.5px] sm:text-[9px] font-bold font-mono px-1.5 py-0.5 rounded-full uppercase shrink-0">
                          {Math.round(((originalPrice - price) / originalPrice) * 100)}% OFF
                        </span>
                      </div>
                      {/* Day count: hidden on mobile, visible on desktop */}
                      <span className="hidden sm:flex text-[8.5px] sm:text-[10.5px] text-slate-500 items-center gap-1 font-mono pt-1 border-t border-slate-200/70">
                    ...
                    <ul className="space-y-1 sm:space-y-1.5 mb-2.5 sm:mb-4 text-[9px] min-[360px]:text-[9.5px] sm:text-xs text-slate-700 flex-1">
                  ...
                  <button
                    ...
                    className="burgundy-btn block w-full py-2 sm:py-2.5 text-center text-[9px] min-[360px]:text-[10px] sm:text-xs font-black rounded-lg sm:rounded-xl uppercase tracking-wider mt-auto shadow-xs group-hover:scale-[1.02] transition-transform cursor-pointer"
                  >
